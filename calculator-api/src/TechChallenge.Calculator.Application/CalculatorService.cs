namespace TechChallenge.Calculator.Application;
...
{
    public async Task<CarbonFootprint> CalculateAsync(string userId, long from, long to, CancellationToken cancellationToken)
    {
        if (from >= to)
        ...
        var readingsTask = measurementsClient.GetReadingsAsync(userId, from, to, cancellationToken);
        var factorsTask = emissionsClient.GetFactorsAsync(from, to, cancellationToken);
